...
class Favorite(DBase):
    ...
    def find_my_favorite(self):
        result = dbsession.query(Favorite,Article).join(Article,Favorite.articleid == Article.articleid)\
            .filter(Favorite.userid == session.get('userid'),Favorite.canceled == 0).all()
        dbsession.close()
        return result
